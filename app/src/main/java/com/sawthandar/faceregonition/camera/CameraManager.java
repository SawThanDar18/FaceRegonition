...
import com.sawthandar.faceregonition.SettingVar;
...
public class CameraManager implements CameraPreview.CameraPreviewListener {
    ...
    private static Camera.Size getBestPreviewSize(Camera mCamera) {
        ...
        float minProportionDiff = getPropotionDiff(widthLargerSize.get(0), propotion);
        ...
        for (int i = 0; i < widthLargerSize.size(); i++) {
            ...
            if (propotionDiff > minProportionDiff) {
                break;
            }
            validSizes.add(size);
        }
    ...
    public static float getPropotionDiff(Camera.Size size, float standardProportion) {
        return Math.abs((float) size.width / (float) size.height - standardProportion);
    }
